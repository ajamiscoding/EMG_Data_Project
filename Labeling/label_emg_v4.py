...
# -*- coding: utf-8 -*-
"""
EMG labeling pipeline — v4 (adaptive filtering + step-by-step plots)
===================================================================
This version *does not* change the agreed pipeline concept. It only adapts
the **filter settings** to the sampling rate (fs) so filters are valid.

Modes (auto-detected from fs unless overridden):
- RAW mode (fs >= 1000 Hz): notch 50 Hz + band-pass 20–450 Hz, then RMS envelope.
- MID mode (200 <= fs < 1000 Hz): notch (if valid) + band-pass 20–min(450, 0.45*fs).
- ENVELOPE mode (fs < 200 Hz, e.g., 100 Hz): treat signal as envelope-like;
  apply high-pass (0.2–0.5 Hz) to remove drift + low-pass (5–10 Hz) to smooth,
  then proceed (rectify+RMS still used as a smoother for consistency).

Saves step-by-step plots and debug CSVs per sheet.
"""
...
class Params:
    ...
    hop_fraction: float = 0.1
    min_on_ms: float = 1000.0
    min_off_ms: float = 100.0
    ...
    bp_low: float = 20.0
    bp_high: float = 450.0
    env_highpass_hz: float = 0.3   # envelope-mode: remove slow drift
    env_lowpass_hz: float = 6.0    # envelope-mode: smooth envelope
...
def process_sheet(df: pd.DataFrame, sheet_name: str, fs: float, time_unit: str, p: Params, out_root: Path, mode: str):
    ...
    for c in range(C):
        ...
        if mode in {"raw", "mid"}:
            # notch first (if valid), then band-pass
            ...
            # high-pass (remove very slow drift), then low-pass (smooth envelope)
    ...
    if len(feats) > 0:
        plt.figure(figsize=(6,5)); plt.scatter(feats[:,0], feats[:,1], s=8, c=labels); plt.xlabel("RMS (log1p)"); plt.ylabel("TKEO (log1p)"); plt.title(f"{sheet_name} — Clusters (RMS vs TKEO)"); plt.tight_layout(); plt.savefig(sheet_dir / "08_clusters_scatter_rms_tkeo.png"); plt.close()

    # 6) Map to sample-wise active
    active = np.zeros(T, dtype=int)
    for lab, (s, e) in zip(labels, zip(starts, ends)):
        if lab == active_label:
            active[s:e] = 1

    plt.figure(figsize=(12,2.5)); plt.step(time, active, where="post"); plt.ylim(-0.2, 1.2); plt.xlabel(f"Time ({time_unit})"); plt.ylabel("Active"); plt.title(f"{sheet_name} — Active (binary)"); plt.tight_layout(); plt.savefig(sheet_dir / "09_active_binary.png"); plt.close()
    ...
    feat_df = pd.DataFrame({
        ...
        "center_time": ((time[starts] + time[np.minimum(ends-1, len(time)-1)]) / 2.0) if len(starts)>0 else [],
...
def build_argparser():
    ap = argparse.ArgumentParser(description="Label EMG attempts per sheet (adaptive filtering + step-by-step plots).")
    ...
    ap.add_argument("--force_mode", type=str, default="auto", choices=["auto","raw","mid","envelope"],
                    help="Override auto mode selection based on fs.")
    return ap
