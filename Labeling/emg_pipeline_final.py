...
# -*- coding: utf-8 -*-
"""
EMG Movement Detection & Analysis Pipeline (FINAL, 1 ms sampling friendly)
...
Main features:
    ...
    • Baseline analysis on envelope per channel.
    • Dead-channel detection (flat channels forced to zero).
    • z-score normalization (for inspection).
    • Windowed RMS per channel (from envelope).
    • Per-channel automatic RMS thresholds (median + k*MAD, capped by percentile).
    • Per-channel activity (RMS > threshold).
    • Global movement from fraction of active channels + pulse detection.

Plots (all stacked over channels where relevant):
    1) Raw EMG
    2) High-pass filtered
    3) Rectified
    4) Envelope
    5) z(t) normalized
    6) Raw with global pulses shaded
    7) RMS per channel + global active shading
    8) Global fraction of active channels + pulses
"""
...
def run_pipeline_on_sheet(
    ...
    hp, rectified, env = preprocess_signals(emg, fs, params)
    ...
    # STEP 4: z-normalization (for inspection)
...
def plot_debug_stages(
    ...
    # 4) Envelope stacked
    _plot_stacked(
        out_dir / f"{sheet_name}_env_stacked.png",
        f"Envelope per Channel (stacked) – {sheet_name}",
        time_s,
        env,
        channel_names,
    )
    ...
    _plot_stacked(
        ...
        f"Normalized z(t) per Channel (stacked) – {sheet_name}",
        time_s,
        z,
        channel_names,
    ...
    # 6) Raw EMG with global pulses shaded
    _plot_stacked(
    ...
    # 7) Window RMS per channel + global_active shading
    if n_windows > 0:
        fig, ax = plt.subplots(figsize=(12, 6))
