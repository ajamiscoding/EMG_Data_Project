...
Per-channel detection + global consensus (no amplitude averaging)
...
Pipeline per sheet:
    ...
    2) Preprocess:
        ...
        - Rectify
        - Low-pass (envelope)
    3) Baseline (rest interval) statistics per channel.
    4) Dead-channel detection (flat / zero channels) → forced to zero.
    5) Normalize to z-score per channel (median & MAD).
    6) Windowing → mean z per window and per channel.
    7) Per-channel automatic activation threshold:
        - From baseline windows of mean z,
        - θ_i = median_i + k * MAD_i  (per-channel).
    8) Per-channel binary activity per window:
        - active_i(w) = 1 if mean_z_i(w) > θ_i.
    9) Global movement signal:
        - channel_count(w) = sum_active_alive_channels,
        - frac_active(w) = channel_count(w) / n_alive.
        - global_active(w) = (frac_active(w) >= consensus_min_fraction).
    10) Pulse detection on frac_active(w) (global movement):
        - Uses pulse_on_z and pulse_off_z (interpreted as fractions 0–1).
    11) Save:
        - baseline.csv (per channel, plus is_dead)
        - pulses.csv (per global pulse)
        - windows.csv (per window: mean_z_i, channel_count, frac_active, global_active)
    12) Plot:
        ...
        - <sheet>_features_consensus.png (mean_z_i + global_active shading)
...
class PipelineParams:
    ...
    # Per-channel automatic threshold: θ_i = median + k * MAD (on baseline windows)
    per_channel_k: float = 2.0
...
def compute_window_features(
    ...
    for k, (start, end) in enumerate(idx):
        segment = z_matrix[start:end, :]
        features[k, :] = np.nanmean(segment, axis=0)
...
# ============================================================
# 7. STAGE 5 – PER-CHANNEL AUTO THRESHOLD + GLOBAL ACTIVITY
# ============================================================

def compute_per_channel_thresholds(
    features: np.ndarray,
    ...
    """
    Compute automatic per-channel thresholds from baseline windows:

    For each channel i:
        1. Take window means z_i(w) in rest region.
...
def per_channel_activity(features: np.ndarray, thresholds: np.ndarray) -> np.ndarray:
    """
    Binary activity per window and channel based on per-channel thresholds.
        active_i(w) = 1 if features[w, i] > thresholds[i]
    """
    return (features > thresholds[None, :]).astype(int)
...
def detect_pulses_on_signal(
    ...
    Uses:
        on_threshold  = params.pulse_on_z
        off_threshold = params.pulse_off_z
    """
...
def run_pipeline_on_sheet(
    ...
    # STEP 6: Per-channel auto thresholds & activity
    print("\nSTEP 6: Per-channel automatic thresholds and activity...")
    thresholds = compute_per_channel_thresholds(
        features,
        t_centers,
    ...
    for ch, th in zip(channel_names, thresholds):
        print(f"    Threshold for {ch:10s}: {th:8.4f}")

    active_all = per_channel_activity(features, thresholds)
    ...
    # Global binary activity (consensus)
    ...
    # Pulse detection on frac_active
...
def plot_debug_stages(
    ...
    Generate plots for each sheet:
      ...
      5) Window mean z per channel + global_active shading
    ...
    if n_windows > 0:
        ...
        # highlight global_active windows
...
def save_results_for_sheet(
    ...
    thresholds: np.ndarray = results["thresholds"]
    is_dead = (~alive_mask).astype(int)

    # Baseline stats + per-channel auto threshold
    baseline_df = pd.DataFrame({
        ...
        "auto_threshold": thresholds, # threshold used for per-channel activation
    })
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="EMG Movement Detection & Analysis Pipeline (per-channel detection + global consensus)"
    )
    parser.add_argument(
        "--in_xlsx",
        type=str,
    ...
    parser.add_argument("--pulse_on_z", type=float, default=0.6)
    parser.add_argument("--pulse_off_z", type=float, default=0.4)
