...
# -*- coding: utf-8 -*-
"""
EMG Movement Detection & Analysis Pipeline (FINAL + baseline gating)

Main ideas:
    • Handles time in ms or seconds (1 ms sampling supported).
    • Per-channel baseline from a rest interval.
    • Normalization: env -> z using (env - median) / MAD.
    • After normalization, tiny |z| values (close to rest) are forced to zero
      => both in z(t) and in env(t) (this is the "gating" your professor suggested).
    • RMS per channel in overlapping windows (from the gated envelope).
    • Per-channel automatic RMS thresholds (median + k*MAD, capped by percentile).
    • Per-channel activity (RMS > threshold), global consensus via fraction of active channels.
    • Pulse detection on the global activity.
    • Stacked plots for each important stage.
...
class PipelineParams:
    ...
    window_ms: float = 200.0     # window length
    overlap_ms: float = 100.0    # overlap between windows
    ...
    # Gating: after normalization, values with |z| < gate_eps are set to zero
    gate_eps: float = 0.3
...
# ============================================================
# 3. INPUT & SAMPLING (robust ms/seconds detection)
# ============================================================
...
def find_time_and_emg_columns(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    ...
    # Heuristic:
    # - If step ~1 and max_raw is large → assume ms (0,1,2,... up to thousands)
...
def normalize_signals(env_matrix: np.ndarray, baseline_stats: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    Robust normalization:
        z(t) = (env(t) - median) / MAD
    """
    m0 = baseline_stats["median"]
    mad = baseline_stats["mad"]
    return (env_matrix - m0[None, :]) / mad[None, :]
...
def apply_gating(
    ...
    """
    After normalization, we gate (zero-out) very small values:
        if |z| < gate_eps → set z = 0 and env = 0.
    This implements the professor's suggestion:
        "after normalization, make near-zero parts exactly zero."
    """
    z_clean = z_matrix.copy()
    env_clean = env_matrix.copy()

    small_mask = np.abs(z_clean) < gate_eps
    z_clean[small_mask] = 0.0
    env_clean[small_mask] = 0.0

    return env_clean, z_clean


# ============================================================
# 5. PREPROCESSING (HPF, rectification, LPF)
# ============================================================
...
def compute_per_channel_rms_thresholds(
    ...
    """
    For each channel i:
      • Take RMS_i(w) in rest region (or all windows if too few).
      • Compute:
            thr_mad  = median + k*MAD
            thr_perc = perc-th percentile (e.g. 85th)
      • Threshold θ_i = min(thr_mad, thr_perc).
      • If channel is dead: θ_i = +∞ (never active).
    """
...
def run_pipeline_on_sheet(
    ...
    hp, rectified, env_raw = preprocess_signals(emg, fs, params)
    print_channel_stats("High-pass filtered", hp, channel_names)
    print_channel_stats("Rectified |HPF|", rectified, channel_names)
    print_channel_stats("Envelope env_raw(t)", env_raw, channel_names)

    # STEP 3: Baseline & dead channels (on raw envelope)
    print("\nSTEP 3: Baseline analysis from rest segment on envelope...")
    rest = select_rest_segment(time_s, env_raw, params.rest_start_sec, params.rest_end_sec)
    baseline_stats = compute_baseline_stats(rest)
    print("  Baseline (median, MAD) per channel on env_raw:")
    ...
    env_raw, baseline_stats, alive_mask = zero_dead_channels(env_raw, baseline_stats, channel_names)

    # STEP 4: z-normalization (from env_raw) + gating (env & z)
    print("\nSTEP 4: Normalization (robust z-score) + gating of near-zero parts...")
    z_raw = normalize_signals(env_raw, baseline_stats)
    print_channel_stats("z_raw(t)", z_raw, channel_names)

    env, z = apply_gating(env_raw, z_raw, gate_eps=params.gate_eps)
    print_channel_stats("Envelope after gating env(t)", env, channel_names)
    print_channel_stats("z(t) after gating", z, channel_names)

    # STEP 5: Window features (RMS + mean z) using gated env & z
...
def plot_debug_stages(
    ...
    emg = results["emg"]
    hp = results["hp"]
    rectified = results["rectified"]
    env_raw = results["env_raw"]
    env = results["env"]
    z = results["z"]
    ...
    # 3) Rectified stacked
    _plot_stacked(
        out_dir / f"{sheet_name}_rectified_stacked.png",
        f"Rectified |HPF| EMG (stacked) – {sheet_name}",
        time_s,
        rectified,
        channel_names,
    )

    # 4) Envelope BEFORE gating
    _plot_stacked(
        out_dir / f"{sheet_name}_env_raw_stacked.png",
        f"Envelope BEFORE gating (stacked) – {sheet_name}",
        time_s,
        env_raw,
        channel_names,
    )

    # 5) Envelope AFTER gating
    _plot_stacked(
        out_dir / f"{sheet_name}_env_gated_stacked.png",
        f"Envelope AFTER gating (stacked) – {sheet_name}",
        time_s,
        env,
        channel_names,
    )
    ...
    # 7) Raw EMG with global pulses shaded
    _plot_stacked(
    ...
    # 8) Window RMS per channel + global_active shading
    if n_windows > 0:
        fig, ax = plt.subplots(figsize=(12, 6))
        ...
        ax.set_ylabel("RMS (gated envelope)")
        ax.grid(True, alpha=0.3)
    ...
    # 9) Global fraction of active channels + pulses
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="EMG Movement Detection & Analysis Pipeline "
                    "(RMS-based per-channel detection + baseline gating + global consensus)"
    )
